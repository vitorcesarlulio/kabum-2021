...
if (isset($_POST['idPeople']) && !empty($_POST['idPeople'])) {
    ...
    $searchContactPeople = $connectionDataBase::connectionDataBase()->prepare($querySelectContactPeople);
    $searchContactPeople->execute($parametros);

    $users = [];
    while ($rowContactPeople = $searchContactPeople->fetch(PDO::FETCH_ASSOC)) {

        # Mascaras Contato, uma opção seria fazer uma função no banco de dados
