...
if (isset($_POST['idUserEdit']) && isset($_POST['nameUserEdit']) && isset($_POST['surnameUserEdit']) && isset($_POST['passwordUserEdit'])) {
    if (!empty($_POST['idUserEdit']) && !empty($_POST['nameUserEdit']) && !empty($_POST['surnameUserEdit']) && !empty($_POST['passwordUserEdit'])) {
        ...
        # Inserindo na tb_usuarios
        $updateUser = $connectionDataBase::connectionDataBase()->prepare($queryUpdateUser);
        ...
        (!empty($permitionUserEdit)) ? $updateUser->bindParam(':usu_permissoes', $user) : $updateUser->bindParam(':usu_permissoes', $admin);

        (!empty($statusActiveUserEdit)) ? $updateUser->bindParam(':usu_status', $active) : $updateUser->bindParam(':usu_status', $inactive);
